  ...
  return (
    <div className={`fixed inset-0 z-50 ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}
      ...
      <div className={`absolute right-0 top-0 h-full w-full max-w-2xl flex flex-col
        bg-gradient-to-b from-slate-900/98 to-slate-950/98 border-l border-blue-500/20
        shadow-[0_0_40px_rgba(59,130,246,0.2)]
        transform transition-transform duration-300 ease-out
        ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>

        {/* ── HEADER ── */}
        <div className="flex-shrink-0 px-6 py-5 border-b border-blue-500/15
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-2.5 min-w-0 flex-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] uppercase tracking-widest text-blue-400/70 font-semibold">AI Insights · DristiScan</span>
              </div>
              {/* title */}
              ...
              <div className="flex items-center gap-2">
                ...
                {explainData?.source_mode === 'rag' && (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg bg-blue-500/10 border border-blue-500/20 text-[10px] text-blue-300">
            ...
            <button type="button" onClick={onClose}
              className="flex-shrink-0 p-2 rounded-lg text-slate-400 hover:text-white hover:bg-blue-500/10 transition mt-0.5">
        ...
        {/* ── TABS ── */}
        <div className="flex-shrink-0 flex items-center gap-1 px-6 py-4 border-b border-blue-500/15 bg-slate-900/80 backdrop-blur-xl">
          {[
            ...
            <button key={id} type="button" onClick={() => setActiveTab(id)}
              className={`relative px-4 py-2 rounded-lg text-sm font-semibold transition-all ${
                activeTab === id
                  ? 'text-white bg-blue-500/15 border border-blue-500/30 shadow-[0_0_12px_rgba(59,130,246,0.2)]'
                  : 'text-slate-400 hover:text-slate-200 border border-transparent hover:border-blue-500/20'
              }`}>
              {label}
              {activeTab === id && (
                <span className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-px h-0.5 w-6 rounded-full bg-blue-400" />
              )}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4
          scrollbar-thin scrollbar-track-transparent scrollbar-thumb-blue-500/20">
