...
def _styles():
    ...
    if _STYLES_CACHE: return _STYLES_CACHE
    ...
    _STYLES_CACHE=b; return b
def _rule(color=None,thickness=0.5): return HRFlowable(width="100%",thickness=thickness,color=color or C_RULE,spaceAfter=6,spaceBefore=2)
def _sp(h=6): return Spacer(1,h)
def _safe(t): return escape(str(t or ""))
def _code_block(code):
    st=_styles(); lines=str(code or "").strip().splitlines()[:40]
    cell=Paragraph(_safe("\n".join(lines)),st["DS_Code"])
    t=Table([[cell]],colWidths=[CONTENT_W])
    t.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),colors.HexColor("#f1f5f9")),("BOX",(0,0),(-1,-1),0.5,C_SLATE_LT),("TOPPADDING",(0,0),(-1,-1),6),("BOTTOMPADDING",(0,0),(-1,-1),6),("LEFTPADDING",(0,0),(-1,-1),8),("RIGHTPADDING",(0,0),(-1,-1),8)]))
    return t
def _info_table(rows):
    ...
    t.setStyle(TableStyle([("VALIGN",(0,0),(-1,-1),"TOP"),("TOPPADDING",(0,0),(-1,-1),4),("BOTTOMPADDING",(0,0),(-1,-1),4),("LEFTPADDING",(0,0),(-1,-1),0),("RIGHTPADDING",(0,0),(-1,-1),6),("LINEBELOW",(0,0),(-1,-2),0.25,C_RULE)]))
    return t
...
def _sev_count_table(critical,high,medium,low):
    ...
    data=[[Paragraph("CRITICAL",hs),Paragraph("HIGH",hs),Paragraph("MEDIUM",hs),Paragraph("LOW",hs)],[Paragraph(str(critical),vs),Paragraph(str(high),vs),Paragraph(str(medium),vs),Paragraph(str(low),vs)]]
    t=Table(data,colWidths=[cw,cw,cw,cw])
    t.setStyle(TableStyle([("BACKGROUND",(0,0),(0,-1),SEV_COLORS["Critical"]),("BACKGROUND",(1,0),(1,-1),SEV_COLORS["High"]),("BACKGROUND",(2,0),(2,-1),SEV_COLORS["Medium"]),("BACKGROUND",(3,0),(3,-1),SEV_COLORS["Low"]),("TOPPADDING",(0,0),(-1,-1),8),("BOTTOMPADDING",(0,0),(-1,-1),8),("LINEAFTER",(0,0),(2,-1),0.5,C_WHITE)]))
    return t
...
class _DSDocTemplate(BaseDocTemplate):
    def __init__(self,buffer,scan_id,file_name,**kw):
        ...
        self.addPageTemplates([PageTemplate(id="Cover",frames=[cf],onPage=self._on_cover),PageTemplate(id="Inner",frames=[inf],onPage=self._on_inner)])
    def _on_cover(self,canvas,doc):
        canvas.saveState(); canvas.setFillColor(C_NAVY); canvas.rect(0,0,PW,PH,fill=1,stroke=0)
        canvas.setFillColor(C_CYAN); canvas.rect(0,0,PW,8*mm,fill=1,stroke=0); canvas.restoreState()
    def _on_inner(self,canvas,doc):
        canvas.saveState(); canvas.setFillColor(C_WHITE); canvas.rect(0,0,PW,PH,fill=1,stroke=0)
        canvas.setFillColor(C_NAVY); canvas.rect(0,PH-12*mm,PW,12*mm,fill=1,stroke=0)
        canvas.setFont("Helvetica-Bold",9); canvas.setFillColor(C_CYAN); canvas.drawString(ML,PH-8*mm,"DristiScan")
        canvas.setFont("Helvetica",7); canvas.setFillColor(C_SLATE_LT); canvas.drawString(ML+28*mm,PH-8*mm,"Security Intelligence Platform")
        canvas.setFont("Helvetica",7); canvas.setFillColor(C_MUTED); canvas.drawRightString(PW-MR,PH-8*mm,(self._file_name or "")[:60])
        canvas.setStrokeColor(C_RULE); canvas.setLineWidth(0.5); canvas.line(ML,MB+8*mm,PW-MR,MB+8*mm)
        ...
        canvas.drawRightString(PW-MR,MB+4*mm,f"Page {doc.page}"); canvas.restoreState()

def _cover_page(scan,structured):
    st=_styles(); s=structured.summary; risk_label=s.overall_risk; risk_color=SEV_COLORS.get(risk_label,C_SLATE)
    ...
    bt.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),risk_color),("TOPPADDING",(0,0),(-1,-1),6),("BOTTOMPADDING",(0,0),(-1,-1),6),("LEFTPADDING",(0,0),(-1,-1),10),("RIGHTPADDING",(0,0),(-1,-1),10)]))
    ms=ParagraphStyle("CM2",fontSize=9,leading=13,textColor=C_SLATE_LT,fontName="Helvetica",alignment=TA_CENTER,spaceAfter=3)
    return [_sp(30),Paragraph("DristiScan",st["DS_Cover_Brand"]),Paragraph("Security Intelligence Platform",st["DS_Cover_Tag"]),
            _sp(20),_rule(C_CYAN_DARK,1),_sp(8),Paragraph("Security Analysis Report",st["DS_Cover_Title"]),_sp(6),
            ...
            _sp(20),_rule(C_NAVY2,0.5),_sp(8),Paragraph(f"Scan ID: #{scan.id}",ms),Paragraph(f"Date: {date_str}",ms),
            Paragraph(f"Total Findings: {s.total}  \u00b7  Critical: {s.critical}  \u00b7  High: {s.high}  \u00b7  Medium: {s.medium}  \u00b7  Low: {s.low}",ms),PageBreak()]

def _toc(sections):
    st=_styles(); story=[*_section_header("Table of Contents"),_sp(4)]
    for num,title in sections:
        row=Table([[Paragraph(num,st["DS_BodySmall"]),Paragraph(title,st["DS_TOC_Entry"])]],colWidths=[12*mm,CONTENT_W-12*mm])
        row.setStyle(TableStyle([("VALIGN",(0,0),(-1,-1),"TOP"),("TOPPADDING",(0,0),(-1,-1),3),("BOTTOMPADDING",(0,0),(-1,-1),3),("LINEBELOW",(0,0),(-1,-1),0.25,C_RULE)]))
        story.append(row)
    story.append(PageBreak()); return story

def _executive_summary(scan,structured):
    st=_styles(); s=structured.summary; rs=structured.risk_score; sec_score=max(0.0,100.0-(rs.score*10))
    scan_date=getattr(scan,"scan_date",datetime.utcnow())
    ...
    story.append(_info_table([("TOTAL FINDINGS",str(s.total)),("OVERALL RISK",s.overall_risk),("RISK SCORE",f"{rs.score:.1f} / 10"),("SECURITY SCORE",f"{sec_score:.0f} / 100"),("SCAN DATE",date_str),("TARGET",scan.file_name or "Unknown")]))
    story.append(_sp(6))
...
def _scan_scope(scan):
    ...
    story.append(_info_table([("TARGET FILE / REPO",scan.file_name or "Unknown"),("SCAN ENGINE","DristiScan v2 \u2014 Rule Engine + AI Agents"),("SCAN TYPE","Static Application Security Testing (SAST)"),("SCAN DATE",str(getattr(scan,"scan_date","N/A"))),("TOTAL ISSUES",str(getattr(scan,"total_findings",0)))]))
    story.append(_sp(4)); story.append(Paragraph("This report covers static analysis of the submitted source code. Dynamic testing and runtime analysis are outside scope.",st["DS_Body"])); story.append(PageBreak()); return story
...
def _risk_overview(structured):
    ...
    if cat_counts:
        ...
        cs=ParagraphStyle("RC",fontSize=8,leading=11,textColor=C_TEXT,fontName="Helvetica"); total=max(s.total,1)
        ...
        t.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),C_NAVY),("ROWBACKGROUNDS",(0,1),(-1,-1),[C_WHITE,C_OFFWHITE]),("GRID",(0,0),(-1,-1),0.25,C_RULE),("TOPPADDING",(0,0),(-1,-1),5),("BOTTOMPADDING",(0,0),(-1,-1),5),("LEFTPADDING",(0,0),(-1,-1),8),("RIGHTPADDING",(0,0),(-1,-1),8),("VALIGN",(0,0),(-1,-1),"MIDDLE")]))
        story.append(t)
    story.append(PageBreak()); return story
...
def _ai_insights_section(structured):
    ...
    for label,value in [("Analysis Summary",ins.summary),("Most Critical Issue",ins.most_critical_issue),("Recommended Fix Priority",ins.fix_priority)]:
        if not value: continue
        story.append(Paragraph(label.upper(),st["DS_Label"]))
        cell=Table([[Paragraph(_safe(value),hs)]],colWidths=[CONTENT_W])
        cell.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),colors.HexColor("#f0f9ff")),("BOX",(0,0),(-1,-1),0.5,C_SLATE_LT),("TOPPADDING",(0,0),(-1,-1),7),("BOTTOMPADDING",(0,0),(-1,-1),7),("LEFTPADDING",(0,0),(-1,-1),10),("RIGHTPADDING",(0,0),(-1,-1),10)]))
...
def _detailed_findings(structured):
    ...
    for idx,finding in enumerate(deduped,1):
        ...
        ns=ParagraphStyle(f"FN{idx}",fontSize=8,leading=10,textColor=C_WHITE,fontName="Helvetica-Bold")
        ts=ParagraphStyle(f"FT{idx}",fontSize=10,leading=13,textColor=C_WHITE,fontName="Helvetica-Bold")
        ss=ParagraphStyle(f"FS{idx}",fontSize=8,leading=10,textColor=C_WHITE,fontName="Helvetica-Bold",alignment=TA_RIGHT)
        hr=Table([[Paragraph(f"{idx:02d}",ns),Paragraph(_safe(finding.type),ts),Paragraph(sev.upper(),ss)]],colWidths=[10*mm,CONTENT_W-30*mm,20*mm])
        hr.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),sc),("TOPPADDING",(0,0),(-1,-1),6),("BOTTOMPADDING",(0,0),(-1,-1),6),("LEFTPADDING",(0,0),(-1,-1),8),("RIGHTPADDING",(0,0),(-1,-1),8),("VALIGN",(0,0),(-1,-1),"MIDDLE")]))
        story.append(hr)
        if finding.line: story.append(Paragraph(f"Line {finding.line}",st["DS_BodySmall"]))
        story.append(_sp(4))
        for label,value in [("DESCRIPTION",finding.description),("IMPACT",finding.impact),("ATTACK EXAMPLE",finding.attack_example),("RECOMMENDATION",finding.recommendation)]:
...
def _secret_section(structured):
    ...
    story.append(Paragraph(f"{len(secrets)} hardcoded secret(s) detected. Rotate all exposed credentials immediately.",st["DS_Body"])); story.append(_sp(6))
    for sf in secrets:
        story.append(Paragraph(f"\u25b8  {_safe(sf.type)}",st["DS_H2"]))
        story.append(_info_table([("SEVERITY",sf.severity),("LINE",str(sf.line) if sf.line else "N/A"),("RECOMMENDATION",sf.recommendation or "Rotate and move to secrets manager.")])); story.append(_sp(6))
    story.append(PageBreak()); return story
...
def _dependency_section(structured):
    ...
    for dep in deps:
        rows.append([Paragraph(_safe(dep.type),cs),Paragraph(_safe(dep.severity),cs),Paragraph(_safe(dep.description[:120]),cs),Paragraph(_safe(dep.recommendation[:80]),cs)])
    t=Table(rows,colWidths=cw)
    t.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),C_NAVY),("ROWBACKGROUNDS",(0,1),(-1,-1),[C_WHITE,C_OFFWHITE]),("GRID",(0,0),(-1,-1),0.25,C_RULE),("TOPPADDING",(0,0),(-1,-1),5),("BOTTOMPADDING",(0,0),(-1,-1),5),("LEFTPADDING",(0,0),(-1,-1),6),("RIGHTPADDING",(0,0),(-1,-1),6),("VALIGN",(0,0),(-1,-1),"TOP")]))
...
def _risk_prioritization(structured):
    ...
    for rank,finding in enumerate(ordered[:15],1):
        ...
        rs2=ParagraphStyle(f"RP{rank}",fontSize=9,leading=11,textColor=C_WHITE,fontName="Helvetica-Bold",alignment=TA_CENTER)
        rt=ParagraphStyle(f"RT{rank}",fontSize=9,leading=12,textColor=C_TEXT,fontName="Helvetica-Bold")
        rv=ParagraphStyle(f"RV{rank}",fontSize=8,leading=10,textColor=C_WHITE,fontName="Helvetica-Bold",alignment=TA_CENTER)
        row=Table([[Paragraph(str(rank),rs2),Paragraph(_safe(finding.type),rt),Paragraph(finding.severity,rv)]],colWidths=[10*mm,CONTENT_W-30*mm,20*mm])
        row.setStyle(TableStyle([("BACKGROUND",(0,0),(0,-1),C_NAVY),("BACKGROUND",(2,0),(2,-1),sc),("BACKGROUND",(1,0),(1,-1),C_OFFWHITE),("TOPPADDING",(0,0),(-1,-1),5),("BOTTOMPADDING",(0,0),(-1,-1),5),("LEFTPADDING",(0,0),(-1,-1),6),("RIGHTPADDING",(0,0),(-1,-1),6),("VALIGN",(0,0),(-1,-1),"MIDDLE"),("LINEBELOW",(0,0),(-1,-1),0.25,C_RULE)]))
        story.append(row)
    story.append(PageBreak()); return story
...
def _compliance_section(structured):
    ...
    owasp_map={"SQL Injection":"A03:2021 \u2013 Injection","Command Injection":"A03:2021 \u2013 Injection","XSS":"A03:2021 \u2013 Injection","Hardcoded":"A02:2021 \u2013 Cryptographic Failures","Secret":"A02:2021 \u2013 Cryptographic Failures","Path Traversal":"A01:2021 \u2013 Broken Access Control","Dependency":"A06:2021 \u2013 Vulnerable Components"}
    ...
    t=Table(rows,colWidths=cw)
    t.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),C_NAVY),("ROWBACKGROUNDS",(0,1),(-1,-1),[C_WHITE,C_OFFWHITE]),("GRID",(0,0),(-1,-1),0.25,C_RULE),("TOPPADDING",(0,0),(-1,-1),5),("BOTTOMPADDING",(0,0),(-1,-1),5),("LEFTPADDING",(0,0),(-1,-1),8),("RIGHTPADDING",(0,0),(-1,-1),8),("VALIGN",(0,0),(-1,-1),"MIDDLE")]))
    story.append(t); story.append(_sp(8)); story.append(Paragraph("References: OWASP Top 10 (2021) \u00b7 NIST NVD \u00b7 MITRE CWE \u00b7 CVE Database",st["DS_BodySmall"]))
    return story

def _get_scan_or_404(db,scan_id):
    scan=db.query(Scan).filter(Scan.id==scan_id).first()
    if not scan: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="Scan not found")
    return scan

def get_report(db,scan_id):
    scan=_get_scan_or_404(db,scan_id)
    return Report(scan_id=scan.id,file_name=scan.file_name,display_file_name=scan.file_name,scan_date=scan.scan_date,total_vulnerabilities=getattr(scan,"total_findings",0),risk_score=getattr(scan,"risk_score",0.0),security_score=getattr(scan,"security_score",0.0),critical_count=getattr(scan,"critical_count",0),high_count=getattr(scan,"high_count",0),medium_count=getattr(scan,"medium_count",0),low_count=getattr(scan,"low_count",0),risk_level=risk_level(scan.risk_score),vulnerabilities=list(scan.vulnerabilities))

def get_structured_report(db,scan_id):
    scan=_get_scan_or_404(db,scan_id); return build_structured_report(scan)

def get_report_pdf(db,scan_id):
    scan=_get_scan_or_404(db,scan_id); structured=build_structured_report(scan); buffer=BytesIO()
    doc=_DSDocTemplate(buffer,scan_id=str(scan.id),file_name=scan.file_name or "",pagesize=A4,leftMargin=ML,rightMargin=MR,topMargin=MT,bottomMargin=MB)
    toc_entries=[("01","Executive Summary"),("02","Scan Scope"),("03","Risk Overview"),("04","AI Insights"),("05","Detailed Findings"),("06","Secret Exposure"),("07","Dependency Vulnerabilities"),("08","Risk Prioritization"),("09","Recommendations"),("10","Compliance & References")]
    story=[NextPageTemplate("Cover"),*_cover_page(scan,structured),NextPageTemplate("Inner"),*_toc(toc_entries),*_executive_summary(scan,structured),*_scan_scope(scan),*_risk_overview(structured),*_ai_insights_section(structured),*_detailed_findings(structured),*_secret_section(structured),*_dependency_section(structured),*_risk_prioritization(structured),*_recommendations_section(structured),*_compliance_section(structured)]
